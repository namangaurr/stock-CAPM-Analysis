...
st.title("Stock Capital Asset Pricing Model ") #title of the website

#user input

col1, col2 = st.columns([1,1])    #column wise display
with col1:
...
try:
    ...
    with col1:
        ...
        st.dataframe(stocks_df.head(), use_container_width =True) #contains the head value
    with col2:
        st.markdown("### DataFrame tail")
        st.dataframe(stocks_df.tail(), use_container_width =True) #contains the tail value
    ...
    with col1:
        ...
        st.plotly_chart(capm_functions.interactive_plot(stocks_df)) #plotting stocks chart

    with col2:
        st.markdown("### Price of all the stocks (After Normalization)")
        st.plotly_chart(capm_functions.interactive_plot(capm_functions.normalize(stocks_df)))#ploting normalized 
    ...
    with col1:
        ...
        st.dataframe(beta_df, use_container_width =True)    #beta value print
